...
serve(async (req) => {
  ...
  try {
    ...
    if (!addresses || !Array.isArray(addresses)) {
      await logApiUsage(supabase, validation.apiKeyId, 'wallet-balances', startTime, 400, req);
      const response: ApiResponse<null> = {
        success: false,
    ...
    const { data: balanceData, error: balanceError } = await supabase.functions.invoke('bitquery-balance', {
      body: { addresses, network }
    });

    if (balanceError) {
      await logApiUsage(supabase, validation.apiKeyId, 'wallet-balances', startTime, 500, req);
      const response: ApiResponse<null> = {
        success: false,
    ...
    await logApiUsage(supabase, validation.apiKeyId, 'wallet-balances', startTime, 200, req);
    ...
    console.error('Error in api-wallet-balances:', error);
    const response: ApiResponse<null> = {
      success: false,
