...
serve(async (req) => {
  ...
  try {
    ...
    if (!address) {
      await logApiUsage(supabase, validation.apiKeyId, 'analyze-wallet', startTime, 400, req);
      const response: ApiResponse<null> = {
        success: false,
    ...
    const { data: analysisData, error: analysisError } = await supabase.functions.invoke('analyze-wallet', {
      body: { address }
    });

    if (analysisError) {
      await logApiUsage(supabase, validation.apiKeyId, 'analyze-wallet', startTime, 500, req);
      const response: ApiResponse<null> = {
        success: false,
    ...
    await logApiUsage(supabase, validation.apiKeyId, 'analyze-wallet', startTime, 200, req);
    ...
    console.error('Error in api-analyze-wallet:', error);
    const response: ApiResponse<null> = {
      success: false,
